...
public class MachineManager {
    ...
    public void order2_showMachineDetails() {
        try {
            ...
            EnigmaStatusDTO status = getMachineStatus();

            System.out.println("Amount of rotors: " + status.getTotalRotors());
            System.out.println("Amount of reflectors: " + status.getTotalReflectors());
            System.out.println("amount of string that encoded: " + status.getTotalProcessedMessages());
            System.out.println("Original positions: " + status.getOriginalCodeCompact());
            System.out.println("Current code: " + status.getCurrentRotorsPositionCompact());
    ...
    public EnigmaStatusDTO getMachineStatus() {
        if (SM.getABC() == null) {
            throw new IllegalStateException("XML File Not Loaded, Storage Are Empty!");
        }

        EnigmaStatusDTO dto = new EnigmaStatusDTO();
        dto.setTotalRotors(SM.getRotorsAmount());
        dto.setTotalReflectors(SM.getReflectorsAmount());

        int processed = (enigmaMachine != null && enigmaMachine.getEngine() != null) ?
                enigmaMachine.getEngine().getNumberOfEncryptions() : 0;
        dto.setTotalProcessedMessages(processed);

        dto.setOriginalCodeCompact(getCode(true));
        dto.setCurrentRotorsPositionCompact(getCode(false));
